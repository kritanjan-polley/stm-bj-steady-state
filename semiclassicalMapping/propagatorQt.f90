module propagator
    ...
    real(kind=wp1), parameter :: l_min_def   = 0.01_wp1
    real(kind=wp1), parameter :: l_max_def   = 0.2_wp1
    real(kind=wp1), parameter :: period = ten
    ...
    subroutine deriv(derivatives_out, state_in, tgs, ek)
        ...
        lam_qc = q_lambda(state_in%qt)
        stql = stwo * lam_qc * state_in%q
        lamda_deriv = stwo * derivative_q_lambda(state_in%qt) * state_in%q
        ...
        derivatives_out%pt = -omega_t * state_in%qt * total_pop - kappa(1)*local_pop(1) &
            - kappa(2)*local_pop(2) - lamda_deriv * off_diag
