module propagator
    ...
    subroutine set_lambda_from_arg(arg_idx)
        ...
        if (nargs >= idx) then
            ...
            read(arg, *) lamda
            write(*,'(a,2x,f10.4)') "Overriding the default value of lamda with", lamda
        else
            write(*,'(a,2x,f10.4)') "No arg provided for lambda, using default:", lamda
        end if
    end subroutine set_lambda_from_arg
    ...
    subroutine set_mu_from_arg(arg_idx)
        ...
        idx = merge(arg_idx, 2, present(arg_idx))
        nargs = command_argument_count()
        if (nargs >= idx) then
            call get_command_argument(idx, arg)
            read(arg, *) mu_left
    ...
    subroutine deriv(derivatives_out, state_in, tgs, ek)
        ...
        stql = stwo * state_in%q * lamda

        h0 =  omega_t * (state_in%pt**2 + state_in%qt**2) * half &
            + omega_c * (state_in%pc**2 + state_in%qc**2) * half
        ...
        do j = 1, jlevel
            ...
            tgs_ybar_sum = mydot(tgs(:, j, 1), state_in%ybar(:, 1)) &
                + mydot(tgs(:, j, 2), state_in%ybar(:, 2))
            tgs_pybar_sum = mydot(tgs(:, j, 1), state_in%pybar(:, 1)) &
                + mydot(tgs(:, j, 2), state_in%pybar(:, 2))
            tgs_xbar_sum = mydot(tgs(:, j, 1), state_in%xbar(:, 1)) &
                + mydot(tgs(:, j, 2), state_in%xbar(:, 2))
            tgs_pxbar_sum = mydot(tgs(:, j, 1), state_in%pxbar(:, 1)) &
                + mydot(tgs(:, j, 2), state_in%pxbar(:, 2))
            derivatives_out%x(j)  = -enj * state_in%y(j) - tgs_ybar_sum &
                - stql_qc * state_in%y(jbar)
            derivatives_out%px(j) = -enj * state_in%py(j) - tgs_pybar_sum &
                - stql_qc * state_in%py(jbar)
            derivatives_out%y(j)  = enj * state_in%x(j) + tgs_xbar_sum &
                + stql_qc * state_in%x(jbar)
            derivatives_out%py(j) = enj * state_in%px(j) + tgs_pxbar_sum &
                + stql_qc * state_in%px(jbar)
        end do

        derivatives_out%q = omega * state_in%p
        derivatives_out%p = -omega * state_in%q - stwo * lamda * off_diag
        derivatives_out%qc = omega_c * state_in%pc * total_pop
        derivatives_out%qt = omega_t * state_in%pt * total_pop
        derivatives_out%pc = -omega_c * state_in%qc * total_pop - (lamda_bar) * off_diag
        derivatives_out%pt = -omega_t * state_in%qt * total_pop &
            - kappa(1)*local_pop(1) - kappa(2)*local_pop(2)

        do l = 1, nel
            do g = 1, glevel
                ekl = ek(g, l)
                derivatives_out%xbar(g, l)  = -ekl * state_in%ybar(g, l) &
                    - dot_product(tgs(g, :, l), state_in%y)
                derivatives_out%pxbar(g, l) = -ekl * state_in%pybar(g, l) &
                    - dot_product(tgs(g, :, l), state_in%py)
                derivatives_out%ybar(g, l)  = ekl * state_in%xbar(g, l) &
                    + dot_product(tgs(g, :, l), state_in%x)
                derivatives_out%pybar(g, l) = ekl * state_in%pxbar(g, l) &
                    + dot_product(tgs(g, :, l), state_in%px)
            end do
        end do
    end subroutine deriv


    pure subroutine single_euler_step(s_out, s_in, k_in, step_size)
    ...
    subroutine rk4(state, tgs, ek)
        ...
        type(system_state), save :: k1, k2, k3, k4
        type(system_state), save :: state_temp
